...
# 2. Information Synthesizer Agent (The "Thinker")
# Role: Process search results and produce a comprehensive, detailed answer.
information_synthesizer_agent = Agent(
    name="Information Synthesizer Agent",
    role="Processes search results to provide a comprehensive and detailed answer.",
    model=llm_model,
    instructions=[
        "You will receive an original user question and relevant search results.",
        "Your task is to synthesize the provided search results to create a comprehensive answer to the original question.",
        "Focus on accuracy and completeness. Do not worry about conciseness or formatting at this stage; provide all relevant details found.",
        "If the search results are insufficient to answer the question comprehensively, state that."
    ],
    show_tool_calls=False, # This agent doesn't use tools
)

# 3. Reasoning Team (The Coordinator / Final Condenser)
...
reasoning_team = Team(
    ...
    model=llm_model,
    members=[web_search_agent, information_synthesizer_agent],
    description="You are an advanced AI coordinator tasked with answering user questions precisely and concisely.",
    instructions=[
        "Your goal is to provide the shortest possible, accurate, and unformatted answer to the user's question.",
        "Follow these steps:",
        "1. **Search:** First, delegate the user's original query to the 'Web Search Agent' to retrieve relevant information.",
        "2. **Synthesize:** Next, delegate the user's original query AND the results from the 'Web Search Agent' to the 'Information Synthesizer Agent'. This agent will provide a detailed answer based on the search results.",
        "3. **Condense and Finalize:** Once you receive the detailed answer from the 'Information Synthesizer Agent', your final and most important task is to **condense that detailed answer into the most concise, direct, and unformatted answer possible that directly addresses the user's original question.**",
